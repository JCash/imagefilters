...
TEST_BEGIN(Example, test_fixture_setup, test_fixture_teardown, test_setup, test_teardown)
    TEST(mytest)
TEST_END(Example)
...
#define TYPENAME_UINT8		"%u"
#define TYPENAME_UINT16		"%u"
#define TYPENAME_UINT32		"%u"
#define TYPENAME_UINT64		"%u"
#define TYPENAME_INT8		"%d"
#define TYPENAME_INT16		"%d"
#define TYPENAME_INT32		"%d"
#define TYPENAME_INT64		"%d"
#define TYPENAME_FLOAT32	"%f"
#define TYPENAME_FLOAT64	"%f"

#define JC_TEST_TYPENAME(x) _Generic((x), \
	uint8_t:	TYPENAME_UINT8,  \
	uint16_t:	TYPENAME_UINT16, \
	uint32_t:	TYPENAME_UINT32, \
	uint64_t:	TYPENAME_UINT64, \
	 int8_t:	TYPENAME_INT8,   \
	 int16_t:	TYPENAME_INT16,  \
	 int32_t:	TYPENAME_INT32,  \
	 int64_t:	TYPENAME_INT64,  \
	  float:	TYPENAME_FLOAT32,\
	 double:	TYPENAME_FLOAT64,\
	default: 	TYPENAME_OTHER)


#define TEST_PRINT_EXPECTED(_A, _B) \
	const char* format = "\n"; \
	switch(JC_TEST_TYPENAME(_A)) { \
	case TYPENAME_UINT8: case TYPENAME_UINT16: case TYPENAME_UINT32: format = "\tExpected %u, got %u\n"; break; \
	case TYPENAME_UINT64: format = "\tExpected %llu, got %llu\n"; break; \
	case TYPENAME_INT8: case TYPENAME_INT16: case TYPENAME_INT32: format = "\tExpected %d, got %d\n"; break; \
	case TYPENAME_INT64: format = "\tExpected %lld, got %lld\n"; break; \
	case TYPENAME_FLOAT32: case TYPENAME_FLOAT64: format = "\tExpected %f, got %f\n"; break; \
	default: break; \
	} \
	JC_TEST_PRINTF(format, _A, _B);
...
int jc_test_run_all_tests(jc_test_state* state)
{
	...
	return state->stats.num_fail > 0 ? 1 : 0;
}
